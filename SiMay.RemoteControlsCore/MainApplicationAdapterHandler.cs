...
namespace SiMay.RemoteControlsCore
{
    public class MainApplicationAdapterHandler : MainApplicationBaseAdapterHandler
    {
        ...
        public void StartService()
        {
            ...
            if (providerType == SessionProviderType.TcpServiceSession)
            {
                if (StartServiceProvider(providerOptions))
                    this.OnLogHandlerEvent?.Invoke($"JCMS远程监控管理系统端口 {port.ToString()} 监听成功!", LogSeverityLevel.Information);
                else
                    this.OnLogHandlerEvent?.Invoke($"JCMS远程监控管理系统端口 {port.ToString()} 启动失败,请检查配置!", LogSeverityLevel.Warning);
            }
            else
            {
                if (StartProxySessionProvider(providerOptions))
                    this.OnLogHandlerEvent?.Invoke($"JCMS远程监控管理系统初始化成功!", LogSeverityLevel.Information);
                else
                    this.OnLogHandlerEvent?.Invoke($"JCMS远程监控管理系统初始化发生错误，请注意检查配置!", LogSeverityLevel.Warning);
            }
        ...
        private void OnActivateStartApp(SessionProviderContext session)
        {
            ...
            string groupName = openControl.GroupName;
            ...
            else
            {
                ...
                if (!context.IsNull())
                {
                    ...
                    appHandlerBase.GroupName = groupName;
        ...
        private void LoginHandler(SessionProviderContext session)
        {
            try
            {
                ...
                this.RemoteActiveService(syncContext, AppJobConstant.REMOTE_DESKTOP_RECORD);
