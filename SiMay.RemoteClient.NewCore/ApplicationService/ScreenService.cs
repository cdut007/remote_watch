...
namespace SiMay.ServiceCore
{
    ...
    [ServiceKey(AppJobConstant.REMOTE_DESKTOP_RECORD)]
    public class ScreenService : ApplicationRemoteService
    {
        ...
        {
            if (_cleanWallPaper)
                User32.SystemParametersInfo(User32.SPI_SETDESKWALLPAPER, 0, _wallpaper, User32.SPIF_UPDATEINIFILE | User32.SPIF_SENDWININICHANGE);
        }

        [PacketHandler(MessageHead.S_SCREEN_RESET_PFRAME)]
        public void ResetPFrame(TcpSocketSaeaSession session)
        {
            var height = _spy.Capturer.Size.Height;
            var width = _spy.Capturer.Size.Width;
            _spy.Capturer.PreviousFrame = new Bitmap(width, height);
        }
        ...
        public void DeleteWallPaper(TcpSocketSaeaSession session)
        {
            ...
            _wallpaper = new string('\0', 260);
            User32.SystemParametersInfo(0x73, 260, _wallpaper, 0);
            _wallpaper = _wallpaper.Substring(0, _wallpaper.IndexOf('\0'));
